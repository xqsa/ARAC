...
def load_design_matrix(file_path):
    """
    Load a design matrix, rebuilding it from metadata if the saved matrix is ragged.

    Some bundled F*-design.txt files are truncated or have a short final row.
    The accompanying F*-info.txt, F*-s.txt, and F*-p.txt files contain enough
    information to reconstruct the same interaction matrix generated by AOBG.
    """
    try:
        return np.loadtxt(file_path, delimiter=',')
    except ValueError as exc:
        if "number of columns changed" not in str(exc):
            raise
        return rebuild_design_matrix_from_metadata(file_path)

def rebuild_design_matrix_from_metadata(file_path):
    """
    Rebuild an AOBG design matrix from F*-info.txt, F*-s.txt, and F*-p.txt.
    """
    design_path = Path(file_path)
    stem = design_path.name.replace("-design.txt", "")
    info_path = design_path.with_name(f"{stem}-info.txt")
    s_path = design_path.with_name(f"{stem}-s.txt")
    p_path = design_path.with_name(f"{stem}-p.txt")

    with open(info_path, "r") as file:
        info = yaml.safe_load(file)

    dimension = int(info["dimension"])
    sub_num = int(info["sub_num"])
    overlap_degree = info.get("overlap_degree", 0)
    subgroups = np.loadtxt(s_path).astype(int)
    pvector = np.loadtxt(p_path, delimiter=",").astype(int)

    if np.isscalar(overlap_degree):
        overlap_list = [int(overlap_degree)] * max(sub_num - 1, 0)
    else:
        overlap_list = [int(value) for value in overlap_degree]

    design_matrix = np.zeros((dimension, dimension), dtype=np.int64)
    begin_index = 0
    for index, subgroup_length in enumerate(subgroups):
        end_index = begin_index + int(subgroup_length)
        subgroup = pvector[begin_index:end_index]
        if len(subgroup) == 0:
            continue
        indices = subgroup - 1
        design_matrix[np.ix_(indices, indices)] = 1
        if index != len(subgroups) - 1:
            begin_index = end_index - overlap_list[index]
