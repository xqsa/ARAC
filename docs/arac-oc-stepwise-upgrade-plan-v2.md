...
设 `R = candidate_error / baseline_error`，越小越好。
...
`shared_neighbor_count`、`handoff_reason`。没有真实 scope acceptance event 时
S2 不能宣称触发，只能标记 `host_unreachable`。
...
- acceptance rate、consumed/returned FE、strict-best 全可审计；
...
失败缩径、self-accept 不 reset、外部变化 reset、v1/v2 state fail-closed。
...
没有 proposal 就 fail-closed，不新增隐式 probe。对 shared coordinate `j`：

```text
consensus_j = weighted mean of owner proposals
d_j = top_owner_proposal_j - second_owner_proposal_j
```

候选只增加 `consensus_j`、`consensus_j + sign(d_j)*r_j` 和
`consensus_j - sign(d_j)*r_j` 三类；候选仍由完整目标 + strict-best 决定。`u_j`
不存在，方向不得由历史积分器生成。
...
直接比较。权重只能影响 consensus candidate 的中心，不得改变 action choice、
scope order、sense/probe/tail budget 或 component 停用。
