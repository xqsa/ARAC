...
class Benchmarks:
    def __init__(self, output_path, data_dir=None):
        ...
        # 评估器
        # fitness_record 默认关闭（2026-08-17 性能补丁）：仓库内无任何
        # 读取方，且每 FE 追加 Python float 在 3M 预算下占用 ~100MB/worker。
        # 需要完整适应度历史时显式置 record_fitness = True。
        self.record_fitness = False
        self.fitness_record = []
        # 热循环常量缓存（2026-08-17 性能补丁）：以下预计算只做常量
        # 提升，运算顺序与数值逐位不变（验收：tmp/vendor_patch_golden.npy）。
        self._asy_coef_cache = {}
        self._elliptic_weight_cache = {}
        self._lambda_exp_cache = {}
        self._conform_index_cache = {}
    ...
    def rotateVectorConform(self, i, c):
        sub_dim = self.s[i]
        indices = self._conform_index_cache.get(i)
        if indices is None:
            start_index = c - i * self.overlap
            end_index = c + sub_dim - i * self.overlap
            indices = self.Pvector[start_index:end_index].copy()
            self._conform_index_cache[i] = indices
        z = self.anotherz[:, indices]
    ...
    def elliptic(self, x):
        nx = x.shape[-1]
        weights = self._elliptic_weight_cache.get(nx)
        if weights is None:
            i = np.arange(nx)
            weights = 10 ** (6 * i / (nx - 1))
            self._elliptic_weight_cache[nx] = weights
    ...
    def transform_asy(self, z, beta=0.2):
        nx = z.shape[-1]
        coef = self._asy_coef_cache.get((nx, beta))
        if coef is None:
            coef = beta * np.arange(nx) / (nx - 1)
            self._asy_coef_cache[(nx, beta)] = coef
        positive_mask = z > 0
        if positive_mask.any():
            cols = np.nonzero(positive_mask)[1]
            zp = z[positive_mask]
            z[positive_mask] = zp ** (1 + coef[cols] * np.sqrt(zp))
        return z

    def Lambda(self, z, alpha=10):
        dim = z.shape[-1]
        exponents = self._lambda_exp_cache.get(dim)
        if exponents is None:
            exponents = 0.5 * np.arange(dim) / (dim - 1)
            self._lambda_exp_cache[dim] = exponents
