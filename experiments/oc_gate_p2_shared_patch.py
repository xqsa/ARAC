"""Gate P2/P3: fresh-seed screening + selector parity (revised plan section 10).

P2 (screening, no superiority claim): fresh seeds 20260890-93, R2/A3/S5/R6,
arms A0-A4.  Checks: contract audits; no pre-registered safety regression
(A4/A1 final ratio <= 1.50 anywhere); A4 non-inferior to A1 (<= 1.05) on
conforming-like cells (operationalized as A4 patch acceptance == 0, i.e. the
patches ran silent); directional results recorded on high-disagreement
cells; real state/radius/reset events observed somewhere; no u overflow and
no patch FE leakage (lane fes only 0/8).

P3 (selector boundary parity): the first selector decision (first receipt
plan hash) must be identical across all arms of a cell; later divergence is
legal once the selector INPUT legitimately changed (recorded, not forced).

Original P1 driver follows below (unchanged).
...
CASES = ("R2", "A3", "S5", "R6")
SEEDS = (20260890, 20260891, 20260892, 20260893)
KNOWN_REGISTRIES = (
...
OUTPUT_ROOT = Path("artifacts/oc_gate_p2_shared_patch")
CELL_SCHEMA = "arac-oc-gate-p2-cell-v1"
OUTPUT_SCHEMA = "arac-oc-gate-p2-v1"
...
def run_gate(workers: int = 6) -> dict[str, object]:
    ...
    checks = {
        ...
        "p3_first_selector_decision_parity": all(
            len({
        ...
        "no_safety_regression": all(
            row["arms"]["A4"]["final_error"]
            <= 1.50 * max(row["arms"]["A1"]["final_error"], 1e-300)
            for row in rows
        ),
        "conforming_cells_noninferior": all(
            row["arms"]["A4"]["final_error"]
            <= 1.05 * max(row["arms"]["A1"]["final_error"], 1e-300)
            for row in rows
            if row["arms"]["A4"]["patch_accepted"] == 0
        ),
        "state_events_observed": any(
            row["arms"]["A4"]["patch_resets"] > 0 or row["arms"]["A4"]["patch_accepted"] > 0
            for row in rows
        ),
        "no_fe_leakage": all(
            row["arms"][arm]["patch_lane_fes"] % 8 == 0
            and row["arms"][arm]["budget_unavailable"] >= 0
            for row in rows
            for arm in ARMS
        ),
    }
    exploratory = [
        {
            "case": row["case_id"],
            "seed": row["seed"],
            "a4_over_a1": row["arms"]["A4"]["final_error"] / max(row["arms"]["A1"]["final_error"], 1e-300),
            "a4_patch_accepted": row["arms"]["A4"]["patch_accepted"],
            "a4_resets": row["arms"]["A4"]["patch_resets"],
            "high_disagreement_directional": row["arms"]["A4"]["patch_accepted"] > 0,
        }
        for row in rows
    ]
    ratios = {
