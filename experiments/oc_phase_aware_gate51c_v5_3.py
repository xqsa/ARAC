"""Gate 51c (v5.3): fresh-seed re-judgment of the verification-ladder scheduler.

OC arms rerun under scheduler v5.3 in a version-isolated directory.  The
standalone arms and the Phase-I checkpoints are reused verbatim from the
frozen v5.1 gate51c run (unchanged anchor); judgment math and protocol
constants are imported unchanged from the v5.1 entry.

Pre-registered v5.3 levers (docs/arac-oc-v5_3-design.md, frozen before
the run): all exploit-class verification windows follow a per-episode
geometric ladder (w1 * 2^rung, capped at segment_fes; material promotes,
flat at the bottom rung releases), and an adopted re-anchor arms a
one-shot release grace absorbing the known warm-up window (Gate 51c v5.2
S5: the adopted CTP's flat 75k window released a polishing chain one
window before its payoff).  Receipts carry the ladder rung, grace
consumption, the deterministic would-release-v5.2 counterfactual, and
the shadow gain rate; the confirmation additionally records the
Phase-I overlap degree per case/seed (v5.4 design input).
"""
...
OUTPUT_ROOT = Path("artifacts/oc_phase_aware_gate51c_v5_3")
...
MANIFEST_FILES = [
    ...
    Path("experiments/oc_phase_aware_gate51c_v5_3.py"),
    Path("docs/arac-oc-gate51c-protocol.md"),
    Path("docs/arac-oc-v5_3-design.md"),
]
...
def _overlap_degree(checkpoint: PhaseCheckpoint) -> float:
    """HCC-style DO: variables in more than one block / D.

    Structurally 0 for ARAC-OC -- the soft-RDDSM leaf partition is
    disjoint by construction; the overlap structure lives in the
    block RELATIONS.  Kept as a recorded structural fact next to
    ``_relation_coupling``, which is the actionable v5.4 input.
    """
    ...
    shared = sum(1 for variable, count in counts.items() if count > 1)
    return shared / max(len(counts), 1)


def _relation_coupling(checkpoint: PhaseCheckpoint) -> dict[str, float]:
    """Variables in relation-carrying blocks / D, plus relation statistics."""

    coupled_blocks: set[int] = set()
    for relation in checkpoint.relations:
        coupled_blocks.add(relation.left_block)
        coupled_blocks.add(relation.right_block)
    coupled_vars = sum(
        len(checkpoint.blocks[index]) for index in coupled_blocks
        if 0 <= index < len(checkpoint.blocks)
    )
    total_vars = sum(len(block) for block in checkpoint.blocks)
    strengths = [relation.strength for relation in checkpoint.relations]
    return {
...
def main() -> int:
    ...
    payload = {
        ...
        "overlap_degree": {
            f"{case}_{seed}": _overlap_degree(
                _parse_phase_one(
                    json.loads(
                        (
                            PREDECESSOR_ROOT / "phase1" / f"{case}_{seed}.json"
        ...
        "relation_coupling": {
            f"{case}_{seed}": _relation_coupling(
                _parse_phase_one(
                    json.loads(
                        (
                            PREDECESSOR_ROOT / "phase1" / f"{case}_{seed}.json"
