"""Run the G0-compatible boundary-gated exact-action pilot (exp_024).

Design (change from exp_023):
- Repair actions with 1 < probe_utility / threshold <= 2 explicitly abstain.
- Higher-utility repair actions and all coordinate actions consume the frozen
  Phase1 candidate exactly; Phase2 never reselects an owner or recomputes a blend.
- Pre-G0 exp_022/023 outcomes are not pooled with this repaired 1000-D protocol.

Cases: E3 (conflicting), S5 (conflicting), R4 (conforming control).
Seeds: same 5 seeds as exp_022/exp_023 for direct comparison.
"""
...
DEFAULT_OUTPUT_ROOT = REPOSITORY_ROOT / "results" / "exp_024_boundary_gated_blend_pilot"
...
def load_config(path: Path) -> dict:
    ...
    assert payload.get("execution", {}).get("relation_policy") == "runtime_probe", \
        "exp_024 requires relation_policy=runtime_probe"
    assert payload.get("execution", {}).get("evidence_overlay_mode") != "off", \
        "exp_024 requires evidence overlay for probe barrier"
    return payload
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"exp_024: {len(tasks)} runs × {execution['max_fes']:,} FE each, {jobs} parallel")
