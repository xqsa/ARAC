"""Run the G0-compatible repair-withheld control pilot (exp_025).

Design (change from exp_023/exp_024):
- Every repair-labeled RuntimeProbeAction is validated at the next sweep and
  explicitly abstained with repair_writeback_withheld.
- Coordinate actions retain the normal G0 exact, one-shot consumption path.
...
Seeds: same 5 seeds as exp_022/exp_023/exp_024 for direct comparison.
"""
...
DEFAULT_OUTPUT_ROOT = REPOSITORY_ROOT / "results" / "exp_025_repair_withheld_control_pilot"
...
def load_config(path: Path) -> dict:
    ...
    assert payload.get("execution", {}).get("relation_policy") == "runtime_probe", \
        "exp_025 requires relation_policy=runtime_probe"
    assert payload.get("execution", {}).get("evidence_overlay_mode") != "off", \
        "exp_025 requires evidence overlay for probe barrier"
    return payload
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"exp_025: {len(tasks)} runs × {execution['max_fes']:,} FE each, {jobs} parallel")
