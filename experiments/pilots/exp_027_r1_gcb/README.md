# exp027: R1 forced phase-boundary GCB validation
...
`gcb` action after three complete native HCC sweeps, consumes it
...
The pilot requires `gcb_action.json` with schema
`gcb-phase-boundary-action-v1`. Validation fails closed unless the artifact records
...
```powershell
python experiments/pilots/exp_027_r1_gcb/run.py
```
...
```powershell
python experiments/pilots/exp_027_r1_gcb/run.py --resume
```

Validate existing output without launching the HCC runner:

```powershell
python experiments/pilots/exp_027_r1_gcb/run.py --reuse-existing
```
