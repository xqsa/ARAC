"""Run the per-relation runtime probe dispatch pilot (exp_022).
...
DEFAULT_OUTPUT_ROOT = REPOSITORY_ROOT / "results" / "exp_022_runtime_probe_dispatch_pilot"
...
def load_config(path: Path) -> dict:
    ...
    assert payload.get("execution", {}).get("relation_policy") == "runtime_probe", \
        "exp_022 requires relation_policy=runtime_probe"
    assert payload.get("execution", {}).get("evidence_overlay_mode") != "off", \
        "exp_022 requires evidence overlay for probe barrier"
    return payload
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"exp_022: {len(tasks)} runs × {execution['max_fes']:,} FE each, {jobs} parallel")
