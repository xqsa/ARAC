"""Gate 38: relative-hub GCB dispatch coordinator vs frozen Gate 29/36 arms."""
...
from experiments.overlap_gcb_coordinator_gate37_screening import (
    _baseline_row,
    _cell_path,
    _find_row,
    _fixed_ctp_row,
...
GATE37 = Path("artifacts/overlap_gcb_coordinator_gate37/confirmation_fresh.json")
CELL_SCHEMA = "arac-overlap-gcb-coordinator-gate38-cell-v1"
OUTPUT_SCHEMA = "arac-overlap-gcb-coordinator-gate38-screen-v1"
...
def _dispatch_summary(result) -> dict[str, object]:
    ...
    return {
        ...
        "max_relative_hub": max(
            (receipt.relative_hub for receipt in receipts), default=0.0
        ),
    }
...
def run_gate(*, workers: int, cell_dir: Path) -> dict[str, object]:
    ...
    return {
        ...
        "protocol": {
            ...
            "baseline_source": "artifacts/overlap_arac_gate29_screening/confirmation_fresh.json",
            "fixed_ctp_source": "artifacts/overlap_coordinate_ctp_gate36/confirmation_fresh.json",
            "calibration_source": "artifacts/overlap_topology_calibration_gate38/calibration.json",
        },
        "summary": {
...
def main() -> int:
    ...
    parser.add_argument(
        "--cell-dir",
        type=Path,
        default=Path("artifacts/overlap_gcb_coordinator_gate38/cells"),
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("artifacts/overlap_gcb_coordinator_gate38/confirmation_fresh.json"),
    )
