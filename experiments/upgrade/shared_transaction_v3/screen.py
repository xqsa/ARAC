"""Screen campaign for shared_transaction_v3 (joint rescue kernel).
...
from experiments.upgrade.shared_transaction_v3.joint_rescue_kernel import JointRescueMount


REPOSITORY_ROOT = Path(__file__).resolve().parents[3]
DEFAULT_PROTOCOL = Path(__file__).with_name("screen_protocol_v1.json")
ARM_SCHEMA = "arac-upgrade-shared-transaction-v3-arm-receipt-v1"
FAILURE_SCHEMA = "arac-upgrade-shared-transaction-v3-failure-v1"
...
def _run_arm(job: ScreenJob) -> dict[str, Any]:
    ...
    try:
        with threadpool_limits(limits=1):
            ...
            from experiments.upgrade.shared_transaction_v3.joint_rescue_kernel import CertifiedLink
...
def summarize(protocol: Mapping[str, Any], output_root: Path) -> dict[str, Any]:
    ...
    body = {
        "schema_version": "arac-upgrade-shared-transaction-v3-summary-v1",
        "candidate_id": "shared_transaction_v3",
...
def run_stage(
    ...
    *,
    ...
    manifest = {
        "schema_version": "arac-upgrade-shared-transaction-v3-manifest-v1",
        "candidate_id": "shared_transaction_v3",
        "freeze_anchor": protocol["freeze_anchor"],
