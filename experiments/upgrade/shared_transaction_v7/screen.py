"""Screen campaign for shared_transaction_v7 (re-centering kernel).
...
from experiments.upgrade.shared_transaction_v7.per_visit_recenter_kernel import PerVisitRecenterMount


REPOSITORY_ROOT = Path(__file__).resolve().parents[3]
DEFAULT_PROTOCOL = Path(__file__).with_name("screen_protocol_v1.json")
ARM_SCHEMA = "arac-upgrade-shared-transaction-v7-arm-receipt-v1"
FAILURE_SCHEMA = "arac-upgrade-shared-transaction-v7-failure-v1"
...
def _run_arm(job: ScreenJob) -> dict[str, Any]:
    ...
    try:
        with threadpool_limits(limits=1):
            ...
            from experiments.upgrade.shared_transaction_v7.per_visit_recenter_kernel import CertifiedLink
...
def summarize(protocol: Mapping[str, Any], output_root: Path) -> dict[str, Any]:
    ...
    body = {
        "schema_version": "arac-upgrade-shared-transaction-v7-summary-v1",
        "candidate_id": "shared_transaction_v7",
...
def run_stage(
    ...
    *,
    ...
    manifest = {
        "schema_version": "arac-upgrade-shared-transaction-v7-manifest-v1",
        "candidate_id": "shared_transaction_v7",
        "freeze_anchor": protocol["freeze_anchor"],
