"""Screen campaign for shared_transaction_v2 (re-centering kernel).
...
from experiments.upgrade.shared_transaction_v2.recentering_kernel import RecenteringMount


REPOSITORY_ROOT = Path(__file__).resolve().parents[3]
DEFAULT_PROTOCOL = Path(__file__).with_name("screen_protocol_v1.json")
ARM_SCHEMA = "arac-upgrade-shared-transaction-v2-arm-receipt-v1"
FAILURE_SCHEMA = "arac-upgrade-shared-transaction-v2-failure-v1"
...
def _run_arm(job: ScreenJob) -> dict[str, Any]:
    ...
    try:
        with threadpool_limits(limits=1):
            ...
            from experiments.upgrade.shared_transaction_v2.recentering_kernel import CertifiedLink

            mount = RecenteringMount(
            ...
            receipts = list(mount.recenter_receipts)
            body = {
                ...
                "kernel_accepted_count": sum(int(item["accepted_count"]) for item in receipts),
...
def summarize(protocol: Mapping[str, Any], output_root: Path) -> dict[str, Any]:
    ...
    for cell_id in protocol["cells"]:
        for seed in protocol["seeds"]:
            ...
            evaluated = sum(
                1
                for receipt in a1["kernel_receipts"]
                for candidate in receipt["candidates"]
                if candidate["evaluated_fes"] > 0
            )
            if not a1["kernel_receipts"] or evaluated < 1:
    ...
    body = {
        "schema_version": "arac-upgrade-shared-transaction-v2-summary-v1",
        "candidate_id": "shared_transaction_v2",
...
def run_stage(
    ...
    *,
    ...
    manifest = {
        "schema_version": "arac-upgrade-shared-transaction-v2-manifest-v1",
        "candidate_id": "shared_transaction_v2",
        "freeze_anchor": protocol["freeze_anchor"],
