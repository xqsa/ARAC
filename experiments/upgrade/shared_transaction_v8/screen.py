"""Screen campaign for shared_transaction_v8 (re-centering kernel).

A0 (mount disabled) vs A1 (re-centering kernel at the T1-qualified SMP
...
from experiments.upgrade.shared_transaction_v1.t3_matched_host_attribution import _load_certified_checkpoint
from experiments.upgrade.shared_transaction_v1.t3_matched_host_attribution import (
    _paired_bootstrap,
    _terminal_contract,
)
from experiments.upgrade.shared_transaction_v8.per_visit_recenter_kernel import PerVisitRecenterMount


REPOSITORY_ROOT = Path(__file__).resolve().parents[3]
DEFAULT_PROTOCOL = Path(__file__).with_name("screen_protocol_v1.json")
ARM_SCHEMA = "arac-upgrade-shared-transaction-v8-arm-receipt-v1"
FAILURE_SCHEMA = "arac-upgrade-shared-transaction-v8-failure-v1"
...
VARIANTS = ("a0", "a1")
STAGE_SEEDS = (20270511, 20270512, 20270513, 20270514, 20270515)
ACTION_SEED = 314159
PHASE1_FES = 180_000
...
def _run_arm(job: ScreenJob) -> dict[str, Any]:
    ...
    try:
        with threadpool_limits(limits=1):
            ...
            from experiments.upgrade.shared_transaction_v8.per_visit_recenter_kernel import CertifiedLink

            mount = PerVisitRecenterMount(
                [CertifiedLink(variable=variable, owner_blocks=(left, right)) for variable, left, right in links],
                enabled=job.variant == "a1",
                phase2_budget=3000000 - 180000,
            )
            ...
            receipts = [mount.ledger_stats.payload()]
            body = {
                ...
                "kernel_consumed_fes": sum(int(item["consumed_fes"]) for item in receipts),
                "kernel_accepted_count": int(receipts[0]["accepted"]) if receipts else 0,
...
def summarize(protocol: Mapping[str, Any], output_root: Path) -> dict[str, Any]:
        ...
        groups.append(
            {
                ...
                "strict_win": bool(
                    ...
                    and float(np.exp(bootstrap["ci95_upper"])) < float(protocol["strict_win_ci_upper"])
                ),
                "breaks_non_inferiority": bool(
                    float(np.exp(bootstrap["ci95_lower"])) > float(protocol["non_inferior_ci_lower"])
                ),
            }
        )
    ...
    for cell_id in protocol["cells"]:
        for seed in protocol["seeds"]:
            a1 = _load_json(output_root / "arms" / cell_id / f"seed_{seed}" / "smp_a1.json")
            stats = a1["kernel_receipts"][0] if a1["kernel_receipts"] else {}
            if int(stats.get("recentered_visits", 0)) < 1 or int(stats.get("consumed_fes", 0)) < 1:
    ...
    body = {
        "schema_version": "arac-upgrade-shared-transaction-v8-summary-v1",
        "candidate_id": "shared_transaction_v8",
...
def run_stage(
    ...
    *,
    ...
    manifest = {
        "schema_version": "arac-upgrade-shared-transaction-v8-manifest-v1",
        "candidate_id": "shared_transaction_v8",
        "freeze_anchor": protocol["freeze_anchor"],
