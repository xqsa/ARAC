"""Screen campaign for shared_transaction_v5 (joint rescue kernel).
...
from experiments.upgrade.shared_transaction_v5.coverage_joint_kernel import JointCoverageMount


REPOSITORY_ROOT = Path(__file__).resolve().parents[3]
DEFAULT_PROTOCOL = Path(__file__).with_name("screen_protocol_v1.json")
ARM_SCHEMA = "arac-upgrade-shared-transaction-v5-arm-receipt-v1"
FAILURE_SCHEMA = "arac-upgrade-shared-transaction-v5-failure-v1"
...
MAX_WORKERS = 8
T1_CTP_ROOT = REPOSITORY_ROOT / "artifacts/upgrade_shared_transaction_v1_t1_v1"
...
class ScreenJob:
    ...
    def receipt_path(self) -> Path:
        return self.output_root / "arms" / self.cell_id / f"seed_{self.run_seed}" / f"ctp_{self.variant}.json"
...
def _run_arm(job: ScreenJob) -> dict[str, Any]:
    ...
    try:
        with threadpool_limits(limits=1):
            ...
            from experiments.upgrade.shared_transaction_v5.coverage_joint_kernel import CertifiedLink

            mount = JointCoverageMount(
                [CertifiedLink(variable=variable, owner_blocks=(left, right)) for variable, left, right in links],
                enabled=job.variant == "a1",
            )
            mount.configure_boundary(boundary_phase="run_persistent_blocks")
...
def summarize(protocol: Mapping[str, Any], output_root: Path) -> dict[str, Any]:
    ...
    for cell_id in protocol["cells"]:
        ...
        for seed in protocol["seeds"]:
            a0 = _load_json(output_root / "arms" / cell_id / f"seed_{seed}" / "ctp_a0.json")
            a1 = _load_json(output_root / "arms" / cell_id / f"seed_{seed}" / "ctp_a1.json")
            for arm in (a0, a1):
    ...
    body = {
        "schema_version": "arac-upgrade-shared-transaction-v5-summary-v1",
        "candidate_id": "shared_transaction_v5",
...
def _frozen_eps_ref() -> dict[str, float]:
    """eps_ref from the T1 patch-off ctp arms (same seeds/action seed)."""

    eps: dict[str, float] = {}
    for cell in CELLS:
        finals = []
        for seed in SEEDS:
            arm = _load_json(T1_CTP_ROOT / "arms" / cell / f"seed_{seed}" / "ctp.json")
...
def run_stage(
    ...
    *,
    ...
    manifest = {
        "schema_version": "arac-upgrade-shared-transaction-v5-manifest-v1",
        "candidate_id": "shared_transaction_v5",
        "freeze_anchor": protocol["freeze_anchor"],
