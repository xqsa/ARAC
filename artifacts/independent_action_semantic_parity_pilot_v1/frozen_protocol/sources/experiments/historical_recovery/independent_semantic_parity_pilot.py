...
DEFAULT_PROTOCOL = Path(__file__).with_name("independent_semantic_parity_protocol.json")
MANIFEST_SCHEMA = "arac-independent-semantic-parity-pilot-manifest-v1"
RECEIPT_SCHEMA = "arac-independent-semantic-parity-pilot-receipt-v1"
SUMMARY_SCHEMA = "arac-independent-semantic-parity-pilot-summary-v1"
SOURCE_PATHS = (
    "experiments/historical_recovery/independent_semantic_parity_pilot.py",
    "experiments/historical_recovery/independent_semantic_parity_protocol.json",
...
def load_protocol(path: Path = DEFAULT_PROTOCOL) -> dict[str, Any]:
    ...
    if protocol["schema_version"] != "arac-independent-action-semantic-parity-protocol-v1":
...
def execute_historical_semantic_port(
    ...
    if context.action_name == "smp":
        ...
        return result, {
            ...
            "stale_window": int(contract["action"]["stale_window"]),
            "state_fields": list(contract["action"]["state_fields"]),
...
def _candidate_mechanism_passed(action: str, events: Mapping[str, Any]) -> bool:
    if action == "aor":
        return (
            ...
            and events.get("initial_mean") == "zero"
...
def _current_reference_match(action: str, route: str) -> bool:
    if action == "smp":
        return route.startswith("stateful_block_visits_")
    return False
...
def _run_lane(lane: Mapping[str, Any], screen_fes: int, output_root_text: str) -> dict[str, Any]:
    ...
    arms["current_production"] = _result_payload(
        ...
        reference_match=_current_reference_match(action, current_result.route),
    )
...
def _summarize(receipts: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    current_matches = sum(
        bool(row["arms"]["current_production"]["reference_mechanism_match"])
        for row in receipts
    )
    ...
    summary = {
        ...
        "current_reference_match_count": current_matches,
        ...
        "lanes": [
            {
                ...
                "current_reference_match": row["arms"]["current_production"][
                    "reference_mechanism_match"
                ],
