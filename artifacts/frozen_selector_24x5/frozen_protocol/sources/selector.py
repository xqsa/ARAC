...
SELECTOR_SCHEMA = "arac-outcome-selector-v1"
OUTCOME_RECORD_SCHEMA = "arac-counterfactual-outcome-v1"
...
class OutcomeSelector:
    ...
    def load(cls, directory: Path) -> OutcomeSelector:
        ...
        if set(getattr(model, "classes_", ())) != set(ACTION_NAMES):
            raise ValueError("selector model was not trained on all actions")
    ...
    def select(
        ...
        action = str(self.model.predict(np.asarray([values], dtype=float))[0])
...
def _training_seed_cv(
    ...
    for seed in seeds:
        ...
        training = ~validation
        if set(targets[training]) != set(ACTION_NAMES):
            raise ValueError("each training-seed fold must retain all four actions")
        model = _new_model()
        model.fit(features[training], targets[training])
        predictions[validation] = model.predict(features[validation])
    metrics = _metrics(records, targets, predictions)
...
def fit_outcome_selector(
    ...
    *,
    ...
    if set(train_y) != set(ACTION_NAMES):
        raise ValueError("outcome training data must contain all four winning actions")
    training_seed_cv = _training_seed_cv(train, train_x, train_y)
    model = _new_model()
    ...
    evaluation = {
        "schema_version": "arac-outcome-selector-evaluation-v1",
        "feature_names": list(PHASE1_FEATURE_NAMES),
        "actions": list(ACTION_NAMES),
        "model_selection": {
            "criterion": "leave-one-training-seed-out strict accuracy",
