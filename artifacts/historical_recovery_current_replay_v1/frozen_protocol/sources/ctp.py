...
def _coverage_fraction(checkpoint: PhaseCheckpoint) -> float:
    relation_ratio = checkpoint.overlap_relation_count / len(checkpoint.blocks)
    return max(
        _MIN_COVERAGE_FRACTION,
        _COVERAGE_FRACTION / (1.0 + relation_ratio),
    )
...
def _relation_cover(context: ActionContext) -> tuple[tuple[int, ...], ...]:
    ...
    for relation in sorted(
        ...
        if len(relation_blocks) >= len(base_blocks):
            break
...
class CtpExecutor:
    ...
    def execute(self, context: ActionContext) -> ActionResult:
        ...
        coverage_budget = min(
            ...
            max(
                sweep_fes,
                int(context.ledger.remaining * _coverage_fraction(context.checkpoint)),
            ),
        )
