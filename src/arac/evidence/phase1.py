...
PHASE1_PROTOCOL = "arac-identity-blind-evidence-v9"
LANDSCAPE_PROBE_FES = 240
PHASE1_MIN_FES = LANDSCAPE_PROBE_FES
PHASE1_FES = 180_000
STRUCTURE_WARMUP_FES = 60_000
...
_WARMUP_SEED_NAMESPACE = 0xE71D3A
_TAIL_SEED_NAMESPACE = 0xA17E5D
_EPSILON = 1e-300
...
def run_phase1(
    ...
    *,
    ...
    warmup_fes = min(
        ...
        max(0, target_fes - ledger.count - STRUCTURE_MIN_FES),
    )
    if warmup_fes:
    ...
    if available >= STRUCTURE_MIN_FES:
        ...
        structural_anchor = ledger.best_x
        structural = infer_structure(
            problem,
            ledger,
            base=structural_anchor,
            base_value=ledger.best_error + problem.optimum,
            run_seed=run_seed,
    ...
    structure_best_error = float(ledger.best_error)
    ...
    progress_features = summarize_phase_progress(
        PhaseProgressErrors(
            ...
            structure=structure_best_error,
