...
class ArchiveSnapshot:
    """Immutable strict-best archive state used by bounded value gates."""

    incumbent: tuple[float, ...]
    error: float

    def __post_init__(self) -> None:
        vector = np.asarray(self.incumbent, dtype=float)
        if vector.ndim != 1 or not np.all(np.isfinite(vector)):
            raise ValueError("archive snapshot incumbent must be a finite vector")
        if not math.isfinite(float(self.error)):
            raise ValueError("archive snapshot error must be finite")
...
class EvaluationLedger:
    ...
    def __init__(
        ...
        *,
        ...
        self.allow_out_of_bounds = bool(allow_out_of_bounds)
    ...
    def archive_snapshot(self) -> ArchiveSnapshot:
        """Capture the strict-best archive without capturing FE history."""

        return ArchiveSnapshot(
            incumbent=tuple(float(value) for value in self.best_x),
            error=float(self.best_error),
        )

    def restore_archive(self, snapshot: ArchiveSnapshot) -> None:
        """Restore an archive after a bounded candidate window is rejected.

        Objective evaluations and their FE count remain consumed. This method
        only restores the incumbent archive, which lets a higher-level value
        gate reject a low-value operator without refunding or hiding FE.
        """

        if not isinstance(snapshot, ArchiveSnapshot):
            raise TypeError("snapshot must be an ArchiveSnapshot")
        vector = np.asarray(snapshot.incumbent, dtype=float)
        if vector.shape != (self.problem.dimension,):
            raise ValueError("archive snapshot dimension does not match the problem")
        if np.any(vector < self._lower_bounds) or np.any(vector > self._upper_bounds):
            raise ValueError("archive snapshot incumbent escaped the problem bounds")
        self._best_x = vector.copy()
        self._best_error = float(snapshot.error)
    ...
    def evaluate(self, candidate: np.ndarray) -> float | np.ndarray:
        ...
        if (
            not self.allow_out_of_bounds
            and (np.any(batch < self._lower_bounds) or np.any(batch > self._upper_bounds))
        ):
    ...
    def evaluate_incumbent(self, *, refresh_error: bool = False) -> float:
        """Evaluate the incumbent once, optionally making that result authoritative."""

        incumbent = self.best_x
        result = float(self.evaluate(incumbent))
        if refresh_error:
            self._best_x = incumbent
            self._best_error = result
        return result
...
__all__ = ["ArchiveSnapshot", "BudgetExceededError", "EvaluationLedger"]
