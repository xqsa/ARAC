...
class _BatchedMMES(_BatchedEvaluation, MMES):
    def __init__(self, problem, options):
        ...
        span = float(np.max(self.upper_boundary - self.lower_boundary))
        self.sigma_floor = np.finfo(float).eps * max(1.0, span)
        self.sigma_floor_repair_count = 0

    def _enforce_sigma_floor(self) -> None:
        sigma = float(self.sigma)
        if not math.isfinite(sigma) or sigma < 0.0:
            raise RuntimeError("MMES sigma became non-finite or negative")
    ...
    def _update_distribution(
        ...
        self._enforce_sigma_floor()
        state = super()._update_distribution(x, mean, p, w, q, t, v, y, y_bak)
        self._enforce_sigma_floor()
        return state
        ...
        *,
        ...
        return OptimizationRun(
            ...
            numerical_repair_count=int(
                getattr(optimizer, "sigma_floor_repair_count", 0)
            ),
        )
...
class ResumableOptimizerSession:
    ...
    def __init__(
        ...
        *,
        ...
        self._dimension_indices = np.asarray(active_dimensions, dtype=int)
        self.anchor = full_anchor.copy()
        self._full_candidate = self.anchor.copy()
        ...
        # Ceiling: a huge-but-finite sigma produces candidates whose square
        # overflows the vendor objectives to inf (observed on R2 at 3M).
        # Clamping sigma to the natural search scale mirrors the floor
        # policy: a counted numerical repair, never a run failure.
        self.sigma_ceiling = float(active_span)
        self.sigma_floor_repair_count = 0
    ...
    def _enforce_sigma_floor(self) -> None:
        ...
        if math.isnan(sigma) or sigma < 0.0:
            raise RuntimeError("optimizer sigma became NaN or negative")
        if sigma == math.inf:
            # The upstream CMA-ES step-size update can overflow to +inf on
            # long AOB-scale runs.  The legacy lanes survive the same event
            # by clipping offspring at the public bounds; the resumable
            # session mirrors that deliberate numerical boundary by
            # collapsing the step size onto its floor (counted as a repair
            # in every receipt) instead of failing the whole run.
        ...
        if sigma < self.sigma_floor:
            self.optimizer.sigma = self.sigma_floor
            self.sigma_floor_repair_count += 1
        elif sigma > self.sigma_ceiling:
            self.optimizer.sigma = self.sigma_ceiling
            self.sigma_floor_repair_count += 1
    ...
    def _evaluate_optimizer_candidate(self, candidate: np.ndarray) -> float | np.ndarray:
        ...
        if single:
            np.copyto(self._full_candidate, self.anchor)
            self._full_candidate[self._dimension_indices] = values
            return self.ledger.evaluate(self._full_candidate)
        full = np.repeat(self.anchor[np.newaxis, :], len(rows), axis=0)
        full[:, self._dimension_indices] = rows
        return self.ledger.evaluate(full)
    ...
    def step(self, budget_fes: int, *, record_trace: bool = True) -> tuple[float, ...]:
        if isinstance(budget_fes, bool) or not isinstance(budget_fes, int) or budget_fes <= 0:
            raise ValueError("budget_fes must be positive")
        ...
        trace: list[float] | None = [] if record_trace else None
    ...
    def restore_state_dict(self, payload: dict[str, object]) -> None:
        ...
        if (
            not math.isfinite(self.optimizer.sigma)
            or self.optimizer.sigma < self.sigma_floor
            or self.optimizer.sigma > self.sigma_ceiling
        ):
            raise ValueError("restored optimizer sigma is outside its numerical floor/ceiling")
