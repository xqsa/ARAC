...
def test_hcc_action_execution_plan_marks_v34_as_downstream_recovery_guard() -> None:
    decision = ActionDecision(
        ActionFamily.TRAJECTORY,
    ...
    assert plan.selected_action_name == "arac_evidence_action_controller_v34"
    assert plan.backend_effect_kind == "evidence_action_runtime_controller_v34"
    assert plan.optimizer_consumed is True
    assert plan.optimizer_consumed_parameters == {
        "relation_runtime_hook": "controller_v33_risk_aware_action_guard",
        "mode_selector": "early_runtime_overlap_relation_evidence_with_relation_first_lock",
    ...
    assert plan.execution_mode == "hcc_evidence_action_controller_v34_runtime_consumed"
    assert plan.runtime_dispatch_allowed is True
