...
def _action(*, acceptance_fitness: float) -> GcbAction:
    ...
    return GcbAction(
        ...
        trigger_context_hash=_hash("e"),
        anchor_hash=gcb_anchor_hash("R4", mean),
        initial_mean=mean,
...
def _execution_context(
    ...
    *,
    candidate_fitness: float,
) -> tuple[GcbExecutionContext, list[int]]:
    ...
    context = GcbExecutionContext(
        ...
        required_seed_namespace=GCB_ACTION,
    )
...
def test_default_dispatcher_routes_gcb_action_and_preserves_hash_contract(
    ...
    assert isinstance(result, GcbExecutionResult)
...
def test_dispatcher_fails_explicitly_for_an_unregistered_action() -> None:
    gcb_action = _action(acceptance_fitness=100.0)
    context, _ = _execution_context(gcb_action, candidate_fitness=10.0)
...
def test_full_space_numerical_execution_has_one_action_layer_owner() -> None:
    ...
    compatibility_source = (
        root / "src" / "arac" / "backends" / "hcc_gcb.py"
    ).read_text(encoding="utf-8")

    assert "execute_gcb_burst_action(" not in runner_source
    assert "sep_optimizer.advance(" not in ceiling_source
    assert "def _execute_gcb_action(" not in compatibility_source
