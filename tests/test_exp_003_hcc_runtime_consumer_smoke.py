...
def test_exp_003_car_w2_diagnostic_profile_has_lazy_lease_controls() -> None:
    from experiments.pilots.exp_003_hcc_runtime_consumer_smoke.run import (
        lanes_for_profile,
        parse_args,
    )

    lanes = lanes_for_profile("car_w2_diagnostic")
    assert [lane.lane_id for lane in lanes] == [
        "v33_fallback",
        "car_w2",
        "car_w2_shuffled",
        "car_w2_paired_fallback",
        "no_action_negative_control",
    ]
    assert [lane.car_candidate_mode for lane in lanes] == [
        "graph",
    ...
    assert parse_args(["--lane-profile", "car_w2_diagnostic"]).lane_profile == (
        "car_w2_diagnostic"
    )
...
def test_exp_003_v38_is_one_precision_reanchor_lane() -> None:
    from experiments.pilots.exp_003_hcc_runtime_consumer_smoke.run import (
        lanes_for_profile,
    )

    lanes = lanes_for_profile("evidence_action_controller_v38")

    assert len(lanes) == 1
    assert lanes[0].runner_action_name == "arac_evidence_action_controller_v38"
    assert lanes[0].selected_action_name == "arac_evidence_action_controller_v38"
    assert lanes[0].plan_action_name == "arac_evidence_action_controller_v38"
    assert lanes[0].relation_dispatch_enabled is True
    assert lanes[0].relation_policy_mode == "controller_v31"


def test_exp_003_cli_accepts_evidence_action_controller_v38_profile() -> None:
    from experiments.pilots.exp_003_hcc_runtime_consumer_smoke.run import parse_args

    args = parse_args(["--lane-profile", "evidence_action_controller_v38"])

    assert args.lane_profile == "evidence_action_controller_v38"


def test_exp_003_v38_trace_schema_reuses_maturity_and_resource_fields() -> None:
    from experiments.pilots.exp_003_hcc_runtime_consumer_smoke.run import (
        V33_TRUST_TRACE_FIELDS,
        V34_RECOVERY_TRACE_FIELDS,
        V36_MATURITY_TRACE_FIELDS,
    ...
    fields = set(
        action_trace_fields_for_lanes(
            lanes_for_profile("evidence_action_controller_v38")
        )
    )
...
def test_exp_003_v39_is_one_cma_sigma_continuation_lane() -> None:
    from experiments.pilots.exp_003_hcc_runtime_consumer_smoke.run import (
        lanes_for_profile,
    )

    lanes = lanes_for_profile("evidence_action_controller_v39")

    assert len(lanes) == 1
    assert lanes[0].runner_action_name == "arac_evidence_action_controller_v39"
    assert lanes[0].selected_action_name == "arac_evidence_action_controller_v39"
    assert lanes[0].plan_action_name == "arac_evidence_action_controller_v39"
    assert lanes[0].relation_dispatch_enabled is True
    assert lanes[0].relation_policy_mode == "controller_v31"


def test_exp_003_cli_accepts_evidence_action_controller_v39_profile() -> None:
    from experiments.pilots.exp_003_hcc_runtime_consumer_smoke.run import parse_args

    args = parse_args(["--lane-profile", "evidence_action_controller_v39"])

    assert args.lane_profile == "evidence_action_controller_v39"


def test_exp_003_v39_trace_schema_adds_only_sigma_continuation_fields() -> None:
    from experiments.pilots.exp_003_hcc_runtime_consumer_smoke.run import (
        V33_TRUST_TRACE_FIELDS,
        V34_RECOVERY_TRACE_FIELDS,
        V36_MATURITY_TRACE_FIELDS,
    ...
    fields = set(
        action_trace_fields_for_lanes(
            lanes_for_profile("evidence_action_controller_v39")
        )
    )
    ...
    assert set(V36_MATURITY_TRACE_FIELDS).issubset(fields)
    assert set(V37_RESOURCE_TRACE_FIELDS).issubset(fields)
    assert set(V39_CMA_SIGMA_TRACE_FIELDS).issubset(fields)
    assert not set(V34_RECOVERY_TRACE_FIELDS).intersection(fields)
